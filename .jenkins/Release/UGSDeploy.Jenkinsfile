...
pipeline {
    parameters {
        ...
        string(name: 'UGS_BUILD_JOB_NAME', defaultValue: 'HorizonFrameworkDemo-UGSBuild', description: 'Name of the UGS build job to copy artifacts from')
    ...
    agent {
        node {
            label 'unreal-win64'
            customWorkspace "${sharedWorkspaceRoot}/HorizonPlugin/HorizonFrameworkDemo/Deploy"
        }
    }
